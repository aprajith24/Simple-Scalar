...
void                              /*mshr create funtion definition, to be place in cache.c*/
mshr_create(int mshr_num)
{
  int i;
  mshr_no = mshr_num;
  dl1_mshr = calloc(mshr_no,sizeof(struct cache_mshr *)*mshr_no);
  for(i=0; i<mshr_no; i++)
  {
    dl1_mshr[i]=calloc(1,sizeof(struct cache_mshr));
  } 

mshr_accesses = 0;
mshr_hits_under_misses = 0;
mshr_hits = 0;
mshr_full = 0;
}

/*stall calculation function declarations for mshr target full and mshr full*/


tick_t mshr_target_full(int i)
{tick_t stall = dl1_mshr[i]->ready;
return stall;
}

tick_t mshr_reg_full()
{ tick_t min_ready = dl1_mshr[0]->ready; 
  int i;
  for(i=0; i<mshr_no; i++){
    if(dl1_mshr[i]->ready < min_ready)
    {min_ready = dl1_mshr[i]->ready;}
  }
return min_ready;
}
		...
		struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  sprintf(buf,"mshr.mshr_accesses");
  stat_reg_counter(sdb, buf, "total number of MSHR accesses", &(mshr_accesses), 0, NULL);

  sprintf(buf,"mshr.hits_under_misses");
  stat_reg_counter(sdb, buf, "total number of hits under misses in MSHR", &(mshr_hits_under_misses), 0, NULL);

  sprintf(buf,"mshr.mshr_hits");
  stat_reg_counter(sdb, buf, "total number of hits in MSHR", &(mshr_hits), 0, NULL);

  sprintf(buf1, "mshr.mshr_hits / mshr.mshr_accesses", name, name);
  sprintf(buf,"mshr.mshr_hit_rate");
  stat_reg_formula(sdb, buf, "hit rate (hits/accesses) in MSHR", buf1, "%12.6f");

  sprintf(buf,"mshr.mshr_full");
  stat_reg_counter(sdb, buf, "total number of full events in MSHR", &(mshr_full), 0, NULL);
...
cache_access(int dl1_flag,   /*flag to check if cache is dl1 cache*/
       tick_t *mem_ready,  /*added parameter for mem_ready*/
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  int mshr_free_flag=0;
  int mshr_block_flag=0;
  int target_free_flag=0;

if(dl1_flag==1)
{
  int i;
  for(i =0; i<mshr_no; i++){
    if(dl1_mshr[i]->ready <= now)
      {dl1_mshr[i]->target_no=0;}
    if(dl1_mshr[i]->block_addr == addr){
      mshr_block_flag=1;
      mshr_accesses++;
      mshr_hits++;
      if(dl1_mshr[i]->target_no < 4){
          dl1_mshr[i]->target_no++;
           target_free_flag=1;
           mshr_hits_under_misses++;
           }
        break;
      }
    }
  if((mshr_block_flag==1) && (target_free_flag==0)){
*mem_ready = mshr_target_full(i);
 return 0;
  }
}
...
int i;
if(dl1_flag==1)
{if(mshr_block_flag==0){
  mshr_accesses++;
   for(i = 0; i<mshr_no; i++){
    if(dl1_mshr[i]->ready <= now)
      {dl1_mshr[i]->target_no=0;}
      if(dl1_mshr[i]->target_no == 0)
      {dl1_mshr[i]->block_addr = addr;
       dl1_mshr[i]->target_no++;  
       mshr_free_flag = 1;
       break;
      }
...
if((mshr_free_flag==0) && (mshr_block_flag==0)) {
*mem_ready = mshr_reg_full();
mshr_full++;
return 0;
}
}
...
if(dl1_flag==1){
  if(mshr_free_flag==1){
  dl1_mshr[i]->ready = now + lat;
  cp->ready = mshr_reg_full();
}
}
