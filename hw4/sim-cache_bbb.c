  ...
  if (cache_dl2)
    {
      ...
      return cache_access(0, NULL, cache_dl2, cmd, baddr, NULL, bsize,
  ...
  if (cache_il2)
    {
      ...
      return cache_access(0, NULL, cache_il2, cmd, baddr, NULL, bsize,
			  /* now */now, /* pudata */NULL, /* repl addr */NULL);
    }
  else
    {
...
#define __READ_CACHE(addr, SRC_T)					\
  ((dtlb								\
    ? cache_access(0, NULL, dtlb, Read, (addr), NULL,				\
		   sizeof(SRC_T), 0, NULL, NULL)			\
    : 0),								\
   (cache_dl1								\
    ? cache_access(0, NULL, cache_dl1, Read, (addr), NULL,			\
		   sizeof(SRC_T), 0, NULL, NULL)			\
    : 0))
...
#define __WRITE_CACHE(addr, DST_T)					\
  ((dtlb								\
    ? cache_access(0, NULL, dtlb, Write, (addr), NULL,				\
		   sizeof(DST_T), 0, NULL, NULL)			\
    : 0),								\
   (cache_dl1								\
    ? cache_access(0, NULL, cache_dl1, Write, (addr), NULL,			\
		   sizeof(DST_T), 0, NULL, NULL)			\
    : 0))
...
{
  if (dtlb)
    cache_access(0, NULL, dtlb, cmd, addr, NULL, nbytes, 0, NULL, NULL);
  if (cache_dl1)
    cache_access(0, NULL, cache_dl1, cmd, addr, NULL, nbytes, 0, NULL, NULL);
  ...
  while (TRUE)
    {
      ...
      if (itlb)
	cache_access(0, NULL, itlb, Read, IACOMPRESS(regs.regs_PC),
		     NULL, ISCOMPRESS(sizeof(md_inst_t)), 0, NULL, NULL);
      if (cache_il1)
	cache_access(0, NULL, cache_il1, Read, IACOMPRESS(regs.regs_PC),
		     NULL, ISCOMPRESS(sizeof(md_inst_t)), 0, NULL, NULL);
