  ...
  if (cache_dl2)
    {
      ...
      lat = cache_access(0, NULL, cache_dl2, cmd, baddr, NULL, bsize,
...
if (cache_il2)
    {
      ...
      lat = cache_access(dl1_flag, NULL, cache_il2, cmd, baddr, NULL, bsize,
			 /* now */now, /* pudata */NULL, /* repl addr */NULL);
      if (cmd == Read)
	return lat;
      else
...
"latency of all pipeline operations.\n"
  ...
  opt_reg_int(odb, "-mshr:size", "number of mshr registers", &mshr_num, 0, TRUE, NULL);
...
mshr_create(mshr_num);       /*Creating dl1 mshr, (mshr_num)number of mshr registers*/
...
struct RUU_station {
  ...
  tick_t ready;          /* time at which an instruction can be issued*/
	      ...
	      if (fu)
		{
		  ...
		  if (cache_dl1)
		    {dl1_flag=1;
		      ...
		      lat =
			cache_access(dl1_flag, &(rs->ready), cache_dl1, Write, (LSQ[LSQ_head].addr&~3),
		  ...
		  if (dtlb)
		    {
		      /* access the D-TLB */
		      lat =
			cache_access(0, NULL, dtlb, Read, (LSQ[LSQ_head].addr & ~3),
				     NULL, 4, sim_cycle, NULL, NULL);
		      if (lat > 1)
			events |= PEV_TLBMISS;
		    }
  ...
  for (n_issued=0;
      ...
      if (RSLINK_VALID(node))
	{
	  ...
	  if (rs->in_LSQ
			      ...
			      == (F_MEM|F_LOAD)))
			{
			  ...
			  if (!load_lat)
				...
				{ dl1_flag=1;
				  ...
				  load_lat =
				    cache_access(dl1_flag, &(rs->ready), cache_dl1, Read,
						 (rs->addr & ~3), NULL, 4,
						 sim_cycle, NULL, NULL);
          dl1_flag=0;  
			  ...
			  if (dtlb && MD_VALID_ADDR(rs->addr))
			    {
				 ...
				 initiate speculative TLB misses */
			      tlb_lat =
				cache_access(0, NULL, dtlb, Read, (rs->addr & ~3),
					     NULL, 4, sim_cycle, NULL, NULL);
			      if (tlb_lat > 1)
				events |= PEV_TLBMISS;
        ...
        if((rs->ready<=sim_cycle) && (cache_dl1->ready<=sim_cycle))
        {
		      /* one more inst issued */
		      n_issued++;
		    }
	  ...
	  if (cache_il1)
	    {
	      /* access the I-cache */
	      lat =
		cache_access(0, NULL, cache_il1, Read, IACOMPRESS(fetch_regs_PC),
	  ...
	  if (itlb)
	    {
		 ...
		 speculative TLB misses */
	      tlb_lat =
		cache_access(0, NULL, itlb, Read, IACOMPRESS(fetch_regs_PC),
			     NULL, ISCOMPRESS(sizeof(md_inst_t)), sim_cycle,
			     NULL, NULL);
	      if (tlb_lat > 1)
